<?php
/**
 * Bot Analytics Dashboard - Clean, organized analytics interface
 *
...
$recent_violations = $rate_limiter->get_rate_limit_violations( 5 );
?>
...
<div class="wrap ta-bot-analytics">
	...
	<p class="description"><?php esc_html_e( 'Track AI bot visits to your markdown content', 'third-audience' ); ?></p>
	...
	<div class="ta-live-feed-widget">
		<div class="ta-live-feed-header">
			<div class="ta-live-feed-title">
				...
				<h2><?php esc_html_e( 'Live Bot Activity', 'third-audience' ); ?></h2>
			</div>
	...
	<div class="ta-summary-cards">
		<div class="ta-summary-card">
			...
			<div class="ta-summary-content">
				...
				<p><?php esc_html_e( 'Total Visits', 'third-audience' ); ?></p>
		...
		<div class="ta-summary-card">
			...
			<div class="ta-summary-content">
				...
				<p><?php esc_html_e( 'Unique Pages', 'third-audience' ); ?></p>
				<span class="ta-summary-meta">
					<?php
					printf(
						/* translators: %d: number of unique bots */
						esc_html__( '%d bots', 'third-audience' ),
						$summary['unique_bots']
					);
					?>
				</span>
		...
		<div class="ta-summary-card">
			...
			<div class="ta-summary-content">
				<h3><?php echo size_format( $summary['total_bandwidth'], 1 ); ?></h3>
				<p><?php esc_html_e( 'Bandwidth', 'third-audience' ); ?></p>
				<span class="ta-summary-meta">
					<?php
					printf(
						/* translators: %s: bandwidth per visit */
						esc_html__( '%s/visit', 'third-audience' ),
						$summary['total_visits'] > 0 ? size_format( $summary['total_bandwidth'] / $summary['total_visits'], 1 ) : '0 B'
					);
					?>
				</span>
			</div>
		...
		<form method="get" id="ta-analytics-filters-form">
			...
			<div class="ta-filter-row">
				<div class="ta-filter-group">
					<input type="date" id="date_from" name="date_from" placeholder="From" value="<?php echo esc_attr( $filters['date_from'] ?? '' ); ?>">
				</div>
				<div class="ta-filter-group">
					<input type="date" id="date_to" name="date_to" placeholder="To" value="<?php echo esc_attr( $filters['date_to'] ?? '' ); ?>">
				</div>
				<div class="ta-filter-group">
				...
				<div class="ta-filter-group">
					<input type="text" id="search" name="search" placeholder="<?php esc_attr_e( 'Search...', 'third-audience' ); ?>" value="<?php echo esc_attr( $filters['search'] ?? '' ); ?>">
				</div>
				<div class="ta-filter-actions">
					<button type="submit" class="button button-primary"><?php esc_html_e( 'Filter', 'third-audience' ); ?></button>
					...
					<button type="button" class="button button-secondary ta-clear-all-visits" style="color: #d63638; border-color: #d63638;">
					...
					<?php foreach ( $bot_stats as $bot ) : ?>
						...
						<div class="ta-legend-item">
							...
							<span class="ta-legend-label">
								...
								<span style="background: <?php echo esc_attr( $priority_badge_color ); ?>; color: white; padding: 2px 6px; border-radius: 4px; font-size: 10px; margin-left: 4px; text-transform: uppercase; font-weight: 600;">
									<?php echo esc_html( $bot_priority_val ); ?>
								</span>
							</span>
							<span class="ta-legend-value"><?php echo number_format( $bot['count'] ); ?></span>
						</div>
					<?php endforeach; ?>
				</div>
			</div>
	...
	<div class="ta-cards-container">
		...
		<div class="ta-card">
			<div class="ta-card-header">
				<h2><?php esc_html_e( 'Top Pages', 'third-audience' ); ?></h2>
			</div>
			<div class="ta-card-body">
				<table class="ta-table">
					...
					<tbody>
						<?php if ( empty( $top_pages ) ) : ?>
							<tr>
								<td colspan="3" class="ta-no-data"><?php esc_html_e( 'No data available', 'third-audience' ); ?></td>
							</tr>
						<?php else : ?>
								...
								<tr>
									<td class="ta-page-title">
										<a href="<?php echo esc_url( $page['url'] ); ?>" target="_blank" title="<?php echo esc_attr( $page['url'] ); ?>">
											<?php echo esc_html( wp_trim_words( $page['post_title'] ?? $page['url'], 6 ) ); ?>
										</a>
									</td>
		...
		<div class="ta-card">
			...
			<div class="ta-card-body">
				...
				<?php else : ?>
					<p class="ta-no-data"><?php esc_html_e( 'No violations recorded', 'third-audience' ); ?></p>
				<?php endif; ?>
			</div>
		</div>
	</div>
	...
	<div class="ta-card">
		<div class="ta-card-header">
			<h2><?php esc_html_e( 'Recent Visits', 'third-audience' ); ?></h2>
			...
			<table class="ta-table">
				...
				<tbody>
					<?php if ( empty( $recent_visits ) ) : ?>
						<tr>
							<td colspan="6" class="ta-no-data"><?php esc_html_e( 'No visits recorded yet', 'third-audience' ); ?></td>
						</tr>
					<?php else : ?>
						<?php foreach ( $recent_visits as $visit ) : ?>
							...
							<tr>
								<td>
									<span class="ta-bot-badge" style="border-left-color: <?php echo esc_attr( $priority_color ); ?>;">
										<?php echo esc_html( $visit['bot_name'] ); ?>
										<span class="ta-priority-indicator" style="background: <?php echo esc_attr( $priority_color ); ?>;">
								...
								<td class="ta-page-cell">
									<a href="<?php echo esc_url( $visit['url'] ); ?>" target="_blank" title="<?php echo esc_attr( $visit['url'] ); ?>">
										<?php echo esc_html( wp_trim_words( $visit['post_title'] ?? $visit['url'], 8 ) ); ?>
									</a>
								</td>
								<td>
			...
			<div class="ta-cache-status-grid">
				<div class="ta-cache-status-card ta-cache-card-success">
					...
					<div class="ta-cache-card-content">
						...
						<p><?php esc_html_e( 'Content served from cache. Fast response (1-5ms)', 'third-audience' ); ?></p>
					</div>
				</div>
				...
				<div class="ta-cache-status-card ta-cache-card-success">
					...
					<div class="ta-cache-card-content">
						...
						<p><?php esc_html_e( 'Fastest response from pre-generated cache (<1ms)', 'third-audience' ); ?></p>
					</div>
				</div>
				...
				<div class="ta-cache-status-card ta-cache-card-warning">
					...
					<div class="ta-cache-card-content">
						...
						<p><?php esc_html_e( 'Generated fresh. Slower response (10-50ms)', 'third-audience' ); ?></p>
					</div>
				</div>
				...
				<div class="ta-cache-status-card ta-cache-card-error">
					...
					<div class="ta-cache-card-content">
						...
						<p><?php esc_html_e( 'Generation failed. Check System Health', 'third-audience' ); ?></p>
					</div>
				</div>
			</div>
		</div>
