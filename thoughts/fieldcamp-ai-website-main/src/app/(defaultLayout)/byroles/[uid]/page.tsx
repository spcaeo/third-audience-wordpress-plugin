...
import "@/app/(defaultLayout)/byroles/byroles.scss";




export default async function DynamicbyrolesPage({ params, searchParams }: { params: { uid: string }, searchParams: { preview?: boolean, p?: string } }) {
  const data = await getPageBySlug('/byroles/'+params.uid, searchParams.preview || false, searchParams.p || '').catch(() => notFound());
      ...
      "@graph": [
        {
          ...
          "itemListElement": [
            ...
            },
            {
              ...
              "position": 2,
              "name": "By Roles",
              "item": "https://fieldcamp.ai/byroles/"
            }
          ]
        }
      ]
...
export async function generateMetadata({ params}:{params:any}) {
  const data = await getPAGESEO('/byroles/'+params.uid);
  return {
