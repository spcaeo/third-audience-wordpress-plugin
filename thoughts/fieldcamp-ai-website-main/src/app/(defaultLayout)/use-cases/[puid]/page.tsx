import { getUsecaseBySlug, getUsecaseSEO, extractImageUrls, getMediaAltByUrls } from '@/lib/api';
...
const UseCaseDetailPage = async ({ params, searchParams }: { params: { puid: string }, searchParams: { preview?: boolean, p?: string, view?: string } }) => {

  const data = await getUsecaseBySlug(`/use-cases/${params.puid}`, searchParams.preview || false, searchParams.p || '').catch(() => notFound());
    if (!data?.content) {
      notFound();
    }
...
export async function generateMetadata({ params}:{params:{ puid: string, uid: string }}) {
  const data = await getUsecaseSEO(`/use-cases/${params.puid}`) 
  return {
