import { getTeamBySlug, getTeamSEO, extractImageUrls, getMediaAltByUrls } from '@/lib/api';
...
const TeamDetailPage = async ({ params, searchParams }: { params: { puid: string }, searchParams: { preview?: boolean, p?: string, view?: string } }) => {

  const data = await getTeamBySlug(`/teams/${params.puid}`, searchParams.preview || false, searchParams.p || '').catch(() => notFound());
    if (!data?.content) {
      notFound();
    }
...
export async function generateMetadata({ params}:{params:{ puid: string, uid: string }}) {
  const data = await getTeamSEO(`/teams/${params.puid}`) 
  return {
