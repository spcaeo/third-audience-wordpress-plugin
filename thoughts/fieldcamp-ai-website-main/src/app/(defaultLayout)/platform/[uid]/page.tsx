...
import StepImageSwitcher from "@/app/(defaultLayout)/platform/StepImageSwitcher";
import ScrollImageSwitcher from "@/app/(defaultLayout)/platform/ScrollImageSwitcher";
import "@/app/(defaultLayout)/platform/platform.scss";




export default async function DynamicplatformPage({ params, searchParams }: { params: { uid: string }, searchParams: { preview?: boolean, p?: string } }) {
  const data = await getPageBySlug('/platform/'+params.uid, searchParams.preview || false, searchParams.p || '').catch(() => notFound());
      ...
      "@graph": [
        {
          ...
          "itemListElement": [
            ...
            },
            {
              ...
              "position": 2,
              "name": "Platform",
              "item": "https://fieldcamp.ai/platform/"
            }
          ]
        }
      ]
...
export async function generateMetadata({ params}:{params:any}) {
  const data = await getPAGESEO('/platform/'+params.uid);
  return {
