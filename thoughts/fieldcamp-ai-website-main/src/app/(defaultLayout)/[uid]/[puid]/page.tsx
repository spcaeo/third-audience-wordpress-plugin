...
export default async function DynamicindustriesPage({ params, searchParams }: { params: { uid: string, puid: string }, searchParams: { preview?: boolean, p?: string } }) {
  const data = await getPageBySlug(params.uid+'/'+params.puid, searchParams.preview || false, searchParams.p || '').catch(() => notFound());
...
export async function generateMetadata({ params}:{params:any}) {
  const data = await getPAGESEO(params.uid+'/'+params.puid);
  return {
