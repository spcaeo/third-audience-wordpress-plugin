...
export default async function page({ params, searchParams }: { params: { uid: string }, searchParams: { preview?: boolean, p?: string } }) {
  ...
  const data = await getPageBySlug(params.uid, searchParams.preview || false, searchParams.p || '').catch(() => notFound());
...
const structuredData = {
 ...
 const replacedContent = data?.content ? parse(data.content, { transform: createTransformFunction(altTextMap) }) : null;
...
export async function generateMetadata({ params}:{params:any}) {
  const data = await getPAGESEO(params.uid) 
  return {
