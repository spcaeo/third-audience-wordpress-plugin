...
const BlogPost = async ({ params, searchParams }: { params: { uid: string }, searchParams: { preview?: boolean, p?: string } }) => {
    ...
    const data = await getSinglePost('alternative', params.uid, searchParams.preview || false, searchParams.p || '').catch(() => notFound());;
      ...
      "@graph": [
        {
          ...
          "itemListElement": [
            ...
            },
            {
              ...
              "position": 2,
              "name": "Alternatives",
              "item": "https://fieldcamp.ai/alternatives/"
            }
          ]
        },
        {
  ...
  switch (layout) {
    ...
    default:
      return (
        <>
          <DefaultBlogLayout data={data} pageType="alternative" altTextMap={altTextMap}/>
          <script
            key={`jobJSON`}
            type="application/ld+json"
            dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData) }}
...
export async function generateMetadata({ params }: { params: any }) {
  const data = await getPOSTSEO("alternative", params.uid);
