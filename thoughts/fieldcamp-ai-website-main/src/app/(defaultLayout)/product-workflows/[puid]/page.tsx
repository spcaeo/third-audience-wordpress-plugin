...
const SalesLeadsTemplate = async ({ params, searchParams }: { params: { puid: string }, searchParams: { preview?: boolean, p?: string, view?: string } }) => {
  ...
  const data = await getPageBySlug(`/product-workflows/${params.puid}`, searchParams.preview || false, searchParams.p || '', 'productWorkflow').catch(() => notFound());
    ...
    <div className="bg-gray-50 font-sans min-h-screen pt-[52px] min-[1023px]:pt-[74px] workflow-template">
      ...
      <div className="bg-white border-b border-gray-200 px-[20px] md:px-6 py-4">
        <div className="flex items-center text-sm text-gray-600 w-full max-w-full xl:max-w-[1245px] 2xl:max-w-[1245px] mx-auto">
          <a href={`${BASE_PATH}/product-workflows/`}>Product Workflows</a>
      ...
      <div className="max-w-6xl mx-auto px-[20px] md:px-6 pb-16 py-16">
        {/* Main Header */}
        <div className="mb-10 banner text-center">
          <div className={`text-center ${data?.featuredImage?.node?.sourceUrl ? 'md:w-[100%]' : ''}`}>
          {/* Back Button */}
          ...
          <p className="text-sm text-gray-500 text-center mt-5 pb-0">By Jeel Patel • Fieldcamp</p>
...
export async function generateMetadata({ params}:{params:{ puid: string, uid: string }}) {
  const data = await getPAGESEO(`/product-workflows/${params.puid}`, 'productWorkflow') 
  return {
