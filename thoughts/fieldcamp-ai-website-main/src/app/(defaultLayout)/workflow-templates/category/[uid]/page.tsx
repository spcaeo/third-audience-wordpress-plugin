...
import { getPOSTSEO, getWorkflowCategories, getWorkflowTemplates } from '@/lib/api';
...
export default async function WorkflowCategoryPage({
  params,
  searchParams,
}: {
  params: { uid: string };
  searchParams?: { [key: string]: string | string[] | undefined };
   ...
   const templates = await getWorkflowTemplates() as WorkflowTemplate[];
   const categories = await getWorkflowCategories() as WorkflowCategory[];
   ...
   const filteredTemplates = templates.filter((template: WorkflowTemplate) => 
     template.workflowCategories?.nodes?.some(
       (cat: WorkflowCategory) => cat.slug === params.uid
     )
   ) || [];
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="w-44 flex-shrink-0">
            <div className="sticky top-[90px]">
              <div className="mb-8">
                <h1 className="text-2xl font-bold text-black mb-0">Workflow Templates</h1>
              </div>
              
              <nav className="space-y-1">
                <Link
                  href={`${BASE_PATH}/workflow-templates/`}
                ...
                {categories?.map((category: WorkflowCategory) => (
                  <Link
                    key={category.slug}
                    href={`${BASE_PATH}/workflow-templates/category/${category.slug}/`}
                  ...
                  {filteredTemplates.map((template: WorkflowTemplate, index: number) => (
                    <div 
                          ...
                          className={`text-card-foreground shadow-sm group hover:shadow-lg transition-all duration-300 border border-gray-200 bg-white rounded-2xl overflow-hidden ${
                            template.workflowCategories?.nodes?.map((category: any) => `category-${category?.slug || 'uncategorized'}`).join(' ') || 'category-uncategorized'
                          }`}
                          data-categories={template.workflowCategories?.nodes?.map((c: any) => c?.slug).filter(Boolean).join(' ') || 'uncategorized'}
                        >
                          ...
                          <div className="absolute top-2 right-2 bg-gray-100 text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity">
                            {template.workflowCategories?.nodes?.map((c: any, i: number) => (
                        ...
                        <div className="p-6 pt-0 pb-6 px-6 flex flex-col items-start space-y-4">
                            <a href={`${BASE_PATH}/workflow-templates/${template.slug}/`}>
...
export async function generateMetadata({ params}:{params:{ puid: string, uid: string }}) {
  const data = await getPOSTSEO('workflowCategory', params.uid) 
  return {
